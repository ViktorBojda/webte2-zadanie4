...
<body>
    <div class="container-xl">
        ...
        <div class="page-content">
            <div class="p-3">
                <h2 class="pb-3">Predpoveď počasia</h2>

                @if (session()->has(['weather', 'country']))
                <div class="row">
                    <div class="col-12 pe-md-5 col-md-4 order-1 order-md-0">
                        <div class="row">
                            <div class="col-12 mb-2">
                                GPS: {{ session('weather')['coord']['lat'] }}, {{ session('weather')['coord']['lon'] }}
                            </div>
                            <div class="col-12 mb-2">
                                Krajina: {{ session('country')['name']['official'] }}
                            </div>
                            <div class="col-12 mb-2">
                                Hlavné mesto: {{ session('country')['capital'][0] }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-4 order-0 order-md-1">
                        <div class="row">
                            <div class="col d-flex justify-content-center fs-4">
                                {{ session('country')['name']['common'] }}, {{ session('weather')['name'] }}
                            </div>
                        </div>
                        <div class="row">
                            <div class="col d-flex justify-content-center">
                                <span class="fs-1">{{ session('weather')['main']['temp'] }}</span>
                                <span class="fs-4">&deg;C</span>
                                <div class="d-flex align-items-end">
                                    <span class="fs-3 ms-3">{{ session('weather')['weather'][0]['main'] }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                @else
                <p>Prejdite na podstránku Adresa a zadajte vašu adresu pre zobrazenie predpovedi počasia.</p>
                @endif
            </div>
